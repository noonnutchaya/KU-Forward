...
class Clothes: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let databaseRef = Database.database().reference()
               databaseRef.child("Clothes").observe(.value, with: { (snapshot) in
...
extension Clothes: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell2 = tableView.dequeueReusableCell(withIdentifier: "cell2",for: indexPath) as? ClothesTableViewCell
        let item = self.items[indexPath.row]
        let url = URL(string: item.profileUrl!)
        let data = try? Data(contentsOf: url!)
        cell2?.NameText.text = item.title
        cell2?.contectText.text = item.contact
        cell2?.priceText.text = item.price! + " บาท"
        cell2?.detailsText.text = item.text
        cell2?.ImgView.image = UIImage(data: data!)
        return cell2!
    }
